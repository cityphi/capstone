\documentclass[../main.tex]{subfiles}
%!TEX root = ./appendixDataSheets.tex
\graphicspath {{../}}

\begin{document}
	
%#####################################################################
%	DATA SHEETS
%#####################################################################
\chapter{Data Sheets} \label{appendix:DataSheets}
\section{Material Properties} \label{materials}
\subsection{3D Printed Nylon 12 \cite{nylon12}} \label{3dProperties}
\begin{figure}[H]
	\centering
	\includegraphics[page={1},width=.8\textwidth]{img/specs/3dPrintedNylon12.pdf}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[page={2},width=\textwidth]{img/specs/3dPrintedNylon12.pdf}
\end{figure}

\subsection{Aluminum 6061 \cite{Aluminum6061}} \label{mat:aluminum6061}
\begin{figure}[H]
	\centering
	\includegraphics[page={1},width=.8\textwidth]{img/specs/aluminum6061.pdf}
\end{figure}

\subsection{Nylon 6 \cite{Nylon6}} \label{mat:nylon6}
\begin{figure}[H]
	\centering
	\includegraphics[page={1},width=.8\textwidth]{img/specs/nylon6.pdf}
\end{figure}

\subsection{316 Stainless Steel \cite{316StainlessSteel}} \label{mat:steel}
\begin{figure}[H]
	\centering
	\includegraphics[page={1},width=.8\textwidth]{img/specs/316StainlessSteel.pdf}
\end{figure}

\subsection{Carbon Fibre Cloth \cite{carbonFibre}} \label{mat:carbonFibre}
\begin{figure}[H]
	\centering
	\includegraphics[page={1},width=.8\textwidth]{img/specs/carbonFibre.pdf}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[page={2},width=.8\textwidth]{img/specs/carbonFibre.pdf}
\end{figure}

%---------------------------------------------------------------------
%	LINEAR ACTUATOR
%---------------------------------------------------------------------
\section{Linear Actuator \cite{LINEARACTUATOR}} \label{LinAct}
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{img/specs/Actuonix+L12+Datasheet.pdf}
\end{figure}

%---------------------------------------------------------------------
%	BEARINGS
%---------------------------------------------------------------------
\section{Bearings \cite{PBEARINGS}} \label{PBearings}
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{img/specs//Bearing-Specs.pdf}
\end{figure}

%---------------------------------------------------------------------
%	FRICTION WHEEL
%---------------------------------------------------------------------
\section{Friction Wheel Assembly}

%	FRICTION WHEEL
\subsection{Friction Wheel \cite{FrictionWheel}} \label{FrictionWheel}
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{img/specs/Friction-Wheel-Specs.pdf}
\end{figure}

%	SET SCREW
\subsection{Friction Wheel Set Screw \cite{SetScrews}} \label{SetScrews}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/specs/Set-Screw.pdf}
\end{figure}

%	MOTOR
\subsection{Friction Wheel Motor \cite{FrictionMotor}} \label{FrictionMotor}
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{img/specs/Friction-Wheel-Motor-Specs.pdf}
\end{figure}

\begin{table}[H]
	\centering
	\caption{Various Gearing For Gondola Motor Torque \cite{FrictionMotor}.}
	\includegraphics[width=\textwidth]{img/specs/motorTorques.PNG}
\end{table}

%	ENCODER
\subsection{Friction Wheel Encoder \cite{RotationalEncoder}} \label{RotationalEncoder}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/specs/Rotational-Encoder.pdf}
\end{figure}

%	CABLE GLAND
\subsection{Example Cable Gland \cite{cableGland}} \label{CableGland}
\begin{figure}[H]
	\centering
	\includegraphics[page={1},width=\textwidth]{img/specs/gland.pdf}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[page={2},width=\textwidth]{img/specs/gland.pdf}
\end{figure}

%---------------------------------------------------------------------
%	KEEL QUOTE
%---------------------------------------------------------------------
\section{Keel Construction Quote} \label{KEELQUOTE}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/specs/keelQuote.pdf}
\end{figure}

%---------------------------------------------------------------------
%	SPRINGS
%---------------------------------------------------------------------
\section{Spring \cite{TORSIONSPRING}} \label{TorsionSpring} \label{TORSIONSPRING}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/specs/Torsion-Spring.pdf}
\end{figure}

%---------------------------------------------------------------------
%	BATTERY
%---------------------------------------------------------------------
\section{Battery \cite{BATTERY}} \label{Battery} \label{BATTERY}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/specs/battery_2250.pdf}
\end{figure}

%---------------------------------------------------------------------
%	THRUSTER
%---------------------------------------------------------------------
\section{Thruster Assembly} \label{Thruster}

%	MOTOR
\subsection{Thruster Motor \cite{ThrusterMotor}} \label{thrusterMotor}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/specs/motor_thruster.pdf}
\end{figure}

%	PROPELLER
\subsection{Propeller \cite{Propeller}} \label{propeller}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/specs/propeller.pdf}
\end{figure}

%	BESC
\enlargethispage{31pt}
\subsection{BESC \cite{BESC}} \label{BESC}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/specs/besc_thruster.pdf}
\end{figure}


%	SERVO
\includepdf[pages=1,pagecommand=\subsection{Servo Motor \cite{ServoVector}} \label{servoMotor}, scale=0.75]{img/specs/vector_servo.pdf}
\includepdf[pages=2, pagecommand={}, scale=0.85]{img/specs/vector_servo.pdf}

%	RECEIVER
\subsection{Receiver \cite{Receiver}} \label{recevier}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/specs/receiver.pdf}
\end{figure}

%---------------------------------------------------------------------
%	FLIGHT CONTROL ASSEMBLY
%---------------------------------------------------------------------
\section{Flight Control Assembly} \label{FlightControl}

%	ESC
\subsection{ESC \cite{ESC}} \label{ESC}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/specs/esc.pdf}
\end{figure}

%	GPS
\subsection{GPS Module \cite{GPSModule}} \label{GPSModule}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/specs/gps.pdf}
\end{figure}

%	TRANSCEIVER
\subsection{Transceiver \cite{Transceiver}} \label{transceiver}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/specs/transceiver.pdf}
\end{figure}

%	FLIGHT CONTROLLER
\subsection{Flight Controller \cite{FlightController}} \label{PIXFalcon}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/specs/flight_controller.pdf}
\end{figure}

\end{document}